...
import com.bochao.auth.util.JsonUtils;
...
public class JwtAccessToken extends JwtAccessTokenConverter{
    ...
    public OAuth2AccessToken enhance(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
        ...
        if(authentication.getPrincipal() instanceof BaseUserDetail) {
	        AuthUser baseAuthUser = ((BaseUserDetail) authentication.getPrincipal()).getBaseUser();
	        // 将用户信息添加到token额外信息中
	        defaultOAuth2AccessToken.getAdditionalInformation().put(Constant.USER_INFO, JSONObject.parseObject(JSONObject.toJSONString(baseAuthUser)));
        }
