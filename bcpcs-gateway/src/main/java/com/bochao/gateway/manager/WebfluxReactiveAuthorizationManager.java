 ...
 */
@Slf4j
@ConfigurationProperties(prefix = "bcpcs.security")
        ...
        return
                ...
                authentication.filter(a ->  a.isAuthenticated())
                    ...
                    .flatMapIterable(a -> {
                        ...
                        //存储当前数据
                        List<Authority> authUsers = new ArrayList<>();
                    ...
                    .any(c-> {//检测权限是否匹配
                        ...
                        List<String> permissions = permissionUtil.getResultPermission(authority);
                        permissions = permissions.stream().filter(permission -> StringUtils.isNotBlank(permission)).collect(Collectors.toList());
                        //请求URl匹配，放行
                        if(permissions.stream().anyMatch(permission -> matcher.match(permission,url))){
                            return true;
                        }
                        return false;
                    })
